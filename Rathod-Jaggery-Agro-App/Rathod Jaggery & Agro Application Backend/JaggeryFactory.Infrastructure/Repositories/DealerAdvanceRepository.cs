...
namespace JaggeryAgro.Infrastructure.Repositories
{
    public class DealerAdvanceRepository : IDealerAdvanceRepository
    {
        ...
        {
            // 1. Initialize query with related entities included to fetch names
            var q = _ctx.DealerAdvances
                        .Include(s => s.Dealer)   // Required to fetch Dealer Name
                        .Include(s => s.PaidBy)   // Required to fetch Employee Name
                        .AsQueryable();

            // 2. Apply Dealer Filter
            if (dealerId.HasValue && dealerId.Value > 0)
            {
                q = q.Where(x => x.DealerId == dealerId.Value);
            }

            // 3. Apply Date Range Filters (Using .Date to ignore time components)
            if (from.HasValue)
            {
            ...
            // 4. Ensure record has an associated payer and return sorted list
            return await q.Where(s => s.PaidBy != null)
